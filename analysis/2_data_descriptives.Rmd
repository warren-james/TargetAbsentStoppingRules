...
Initially, we centred previous RTs according to the grand mean 
...
Difficulty was divided by pi in order to make this measure scaled. 0 being the least amount possible and 1 being the most difficult amount of variation. Though we didn't approach these valuse (as a value of one here would make the task impossible), it's good to have this standardised. 

Now, 0 in difficulty means no variation in the distractors, 1 means they are uniformly random

```{r scale difficulty}
df$s_difficulty <- df$difficulty/pi

df$s_difficulty <- round(df$s_difficulty, digits = 3)
```

Target presence is converted using "as.numeric()". This changed 0 -> 1 and 1 -> 2, so we subtracted 1 from everything in this column

```{r use as.numeric() on targ_pr}
df$targ_pr <- as.numeric(df$targ_pr)

df$targ_pr <- df$targ_pr - 1
```
