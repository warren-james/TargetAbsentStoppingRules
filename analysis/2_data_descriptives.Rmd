...
mean_acc_plot = mean_acc_plot + labs(x = "Difficulty", y = "Proportion correct responses", title = "Average Accuracy in Target present (1) and Target Absent (0) trials.")
...
``` {r working on density plots for TA trials as a probability of TP rts, include = FALSE}
...
# set up the data frame for this part
# We only want correct trials, so we should use that for now
# though, we might want to include incorrect trials for the TA part and separate on that?
df_pnorm <- df_correct_only
...
# now create the loop
for(th in unique(df_pnorm$difficulty)){
  for(p in unique(df_pnorm$participant)){
    d <- prob_data(df_pnorm, p, th)
    
    # add into datafram 
    df_pnorm[which(df_pnorm$participant == p & df_pnorm$difficulty == th),] <- d
  }
  # create filename
  filename <- paste("scratch/plots/diff_", th, "_probability_plots.pdf", sep = "")
  
  # make the plot 
  plt = ggplot(df_pnorm[df_pnorm$difficulty == th,], aes(pnormRT, colour = TATP))
  plt = plt + geom_density(alpha = 1)
  plt = plt + scale_y_continuous(limits = c(0,9))
