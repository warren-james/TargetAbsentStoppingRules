...
# add in previous presence or absence for random #
people <- seq(max(as.numeric(alldat$participant))) #This will need to be changed to the total number of participants...
count = 0 
n_to_shift <- 1
...
for(Subject in people){
  ...
  for (block_no in nblocks){
    ...
    if (count == 1)
    {
      all_data_with_Ptargpres_ra <- data_this_block #same as before, makes new data set
    }
    if (count > 1)
    {
      all_data_with_Ptargpres_ra <- rbind(all_data_with_Ptargpres_ra,data_this_block)
    }
...
all_data_with_PTP <- rbind(all_data_with_Ptargpres_bl, all_data_with_Ptargpres_ra)
rm(all_data_with_Ptargpres_bl, all_data_with_Ptargpres_ra)
...
# add in previous difficulty for random #
people <- seq(max(as.numeric(alldat$participant))) #This will need to be changed to the total number of participants...
count = 0 
n_to_shift <- 1
...
for(Subject in people){
  ...
  for (block_no in nblocks){
    ...
    if (count == 1)
    {
      all_data_with_Prediff_ra <- data_this_block #same as before, makes new data set
    }
    if (count > 1)
    {
      all_data_with_Prediff_ra <- rbind(all_data_with_Prediff_ra,data_this_block)
    }
...
all_data_with_Prediff <- rbind(all_data_with_Prediff_bl, all_data_with_Prediff_ra)
...
rm(all_data_with_Prediff_bl,all_data_with_Prediff_ra)
...
zradat <- alldat[alldat$Block_Type == " ra",]
...
# add in previous z-score for random #
people <- seq(max(as.numeric(alldat$participant))) #This will need to be changed to the total number of participants...
count = 0 
n_to_shift <- 1
...
for(Subject in people){
  ...
  data_this_sub <- zradat[zradat$participant == Subject,]
  nblocks <- unique(data_this_sub$block)
  for (block_no in nblocks){
    ...
    if (count == 1)
    {
      all_data_with_zprev_ra <- data_this_block #same as before, makes new data set
    }
    if (count > 1)
    {
      all_data_with_zprev_ra <- rbind(all_data_with_zprev_ra,data_this_block)
    }
...
all_data_with_zprev <- rbind(all_data_with_zprev_bl, all_data_with_zprev_ra)

rm(all_data_with_zprev_bl,all_data_with_zprev_ra)
...
rm(alldat,randomFiles, blockFiles, zradat, zdat, zbldat, radat, radat2, PZdat, condsd, bldat2, bldat, all_data_with_zprev, all_data_with_Prediff,f,block_no, nblocks, n_to_shift, Subject, Prediff, PTPdat, people, PCTdat,count)
